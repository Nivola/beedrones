...
class VsphereClientTestCase(BeedronesTestCase):
    ...
    def test_sslvpn_user_modify(self):
        ...
        res = self.client.network.nsx.edge.sslvpn_user_modify(edgeid, user, description="CSI User", last_name="Bello")
        self.logger.info(res)

    def test_sslvpn_private_network_modify(self):
        global edgeid

        res = self.client.network.nsx.edge.sslvpn_private_network_modify(
            edgeid, network_cidr="172.25.6.0/24", optimize=True
        )
        self.logger.info(res)
    ...
    def test_nsx_create_ippool(self):
        global oid
        res = self.client.network.nsx.ippool.create(
            "prova-ippool",
            prefix=24,
            dnssuffix="tstsddc.csi.it",
            dns1="10.102.184.2",
            dns2="10.102.184.3",
            startip="10.102.185.50",
            stopip="10.102.185.60",
            gateway="10.102.185.1",
        )
        self.logger.info(res)
        oid = res
            ...
            "vnics": [
                {
                    ...
                    "addressGroups": [{"primaryAddress": "192.168.3.2", "subnetPrefixLength": 24}],
                }
            ],
    ...
    def test_nsx_add_edge_firewall_rules(self):
        ...
        res = self.client.network.nsx.edge.firewall_rule_add(
            edgeid,
            "rule01",
            "accept",
            logged=True,
    ...
    def test_nsx_add_edge_nat_rules(self):
        ...
        res = self.client.network.nsx.edge.nat_rule_add(
            edgeid,
            desc,
            action,
            original_address,
    ...
    def test_nsx_add_edge_vnic(self):
        ...
        kvargs = {
            ...
            "addressGroups": [{"primaryAddress": "192.169.3.2", "subnetPrefixLength": 24}],
        }
    ...
    def test_nsx_edge_lb_enable_true(self):
        res = self.client.network.nsx.edge.lb.update_global_config(
            "edge-57",
            edgeLogging="true",
    ...
    def test_nsx_edge_lb_add_app_profile(self):
        res = self.client.network.nsx.edge.lb.add_app_profile(
            "edge-57", "TCP", "msrdp_by_api", persistence="msrdp", expire=120
        )
        self.logger.info(res)

    def test_nsx_edge_lb_update_app_profile(self):
        res = self.client.network.nsx.edge.lb.update_app_profile(
            "edge-57",
            "applicationProfile-42",
            name="pluto e minnie",
    ...
    def test_nsx_edge_lb_add_pool(self):
        res = self.client.network.nsx.edge.lb.add_pool(
            "edge-57",
            "prova_by_api4",
            "URl",
    ...
    def test_nsx_edge_lb_add_pool_member(self):
        res = self.client.network.nsx.edge.lb.add_pool_member(
            "edge-57",
            "pool-2",
    ...
    def test_nsx_create_dfw_rule(self):
        ...
        appliedto = None
        res = self.client.network.nsx.dfw.create_rule(
            oid, "prova", "allow", "out", False, source, destination, service, appliedto
        )
        self.logger.info(res)
        rid = res["id"]
    ...
    def test_create_server(self):
        ...
        task = self.client.server.create(
            name,
            guest_id,
            datastore,
            folder,
    ...
    def test_server_guest_execute_command(self):
        ...
        server = self.__get_server("vm-prova-01")
        pwd = self.params.get("server_pwd")
        proc = self.client.server.guest_execute_command(
            server, "root", pwd, path_to_program="/bin/echo", program_arguments=cmd
        )
