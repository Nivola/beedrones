...
class OpenstackSwiftTestCase(CloudapiTestCase):
    ...
    def test_account_metadata_post(self):
        res = client.swift.account_metadata_post(
            x_account_meta_name={"prova": "prova", "prova2": ""},
        )
    ...
    def test_container_metadata_post(self):
        container = "prova"
        res = client.swift.container_metadata_post(
            container=container,
            x_container_meta_name={
                "meta1": "container_metadata_post",
                "meta2": "container_metadata_post",
            },
        )
        self.logger.debug("Create openstack swift container %s: %s" % (container, jsonDumps(res)))
    ...
    def test_object_copy(self):
        container = "prova"
        c_object = "test3"
        res = client.swift.object_copy(container=container, c_object=c_object, destination="prova/test4")
        self.logger.debug("Put openstack swift object: %s" % jsonDumps(res))
    ...
    def test_object_metadata_post(self):
        container = "prova"
        c_object = "test4"
        res = client.swift.object_metadata_post(
            container=container,
            c_object=c_object,
            x_object_meta_name="Metadata name prova",
            content_type="text/html; charset=UTF-8",
        )
        self.logger.debug("Modify openstack swift object metadata: %s" % jsonDumps(res))

    def test_generate_key(self):
        res = client.swift.generate_key(key="containerkey", container="signaling")
        self.logger.debug("Generate openstack swift temp key: %s" % jsonDumps(res))
    ...
    def test_generate_temp_url(self):
        ...
        method = "GET"
        key = "containerkey"
        res = client.swift.generate_temp_url(c_object=c_object, timeout=timeout, method=method, key=key)
        self.logger.debug("Generate openstack swift temp urls: %s" % res)
