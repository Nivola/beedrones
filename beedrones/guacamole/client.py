...
class GuacamoleClient(object):
    def __init__(self, hostname, username, password, method='https',
        ...
        if default_datasource:
            ...
            if default_datasource not in self.datasources:
                self.logger.error('Datasource {} does not exist. Possible values: {}'.format(
                    default_datasource, self.datasources))
                raise GuacamoleError('Datasource {} does not exist. Possible values: {}'.format(
                    default_datasource, self.datasources))
    ...
    def __authenticate(self):
        ...
        self.logger.debug("""requests.post( url=%s/tokens,data={'username': %s, 'password': %s},verify=%s,
            allow_redirects=True)""" % (self.REST_API, self.username, self.password, self.verify))
        r.raise_for_status()
    ...
    def __auth_request(self, method, url, payload=None, url_params=None,
        ...
        self.logger.debug('requests.request(method={}, url={}, params={}, json={}, verify={}, allow_redirects=True'
                          .format(method, url, params, payload, self.verify))
