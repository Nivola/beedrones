...
class WorkFlowEngine(object):
    ...
    def __init__(self, conn, user='anonymous', passwd='anonymous', proxy=None, keyfile=None, certfile=None):
        """
    ...
    def process_deployment_create(self, source, name, type="bpmn", checkduplicate=True):
        ...
        path = self.connection["proto"] + "://" + str(self.connection["host"]) + ":" + str(self.connection["port"]) + \
    ...
    def process_deployment_delete(self, processDeploymentId, cascade=True, skipCustomListeners=False):
        ...
        if cascade is True:
            ...
            for process in processes:
                ...
                self.process_definition_delete(processDefinitionId=processDefinitionId, cascade=True, 
    ...
    def process_instances_list(self, processDefinitionKey=None, businessKey=None, processInstanceIds=None):
        ...
        return self.client.run_http_request2('/process-instance', 'POST', data=data,
                                             headers={'Content-Type': 'application/json'})
    ...
    def process_instance_variable_file_upload(self, processInstanceId, varName, varContent):
        ...
        path = self.connection["proto"] + "://" + str(self.connection["host"]) + ":" + str(self.connection["port"]) + \
            self.connection["path"] + '/process-instance/' + processInstanceId + '/variables/' + varName + '/data'
    ...
    def tasks (self, account_id=None, instance_id=None):
        ...
        for task in tasks:
            result.append(
                {
                    u'instance_id': task["formKey"],
                    u'task_id': task["id"],
                    u'execution_id': task["executionId"],
                    u'task_name': task["name"],
                    u'due_date': format_date(strdate_to_date(task["due"])),
                    u'created':  format_date(strdate_to_date(task["created"])),
                })
        return result
    ...
    def task_complete(self, task_id, variables=None):
        ...
        return self.client.run_http_request2('/task/' + task_id + '/complete', 'POST', data=data,
                                             headers={'Content-Type': 'application/json'})
    ...
    def externaltask_fetch_and_lock(self, workerid, maxtasks, topicname, usepriority=True, lockduration=360000):
        ...
        result = self.client.run_http_request2('/external-task/fetchAndLock', 'POST', data=data,
                                               headers={'Content-Type': 'application/json'})
