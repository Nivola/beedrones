    ...
    def connect(self):
        try:
            if self.user is not None:
                conn_string = "qemu+ssh://%s@%s/system?no_tty=1&keyfile=%s" % (
                    self.user,
                    self.host,
                    self.key,
                )
            ...
            conn.connect()
            self.logger.debug("Get libvirt-qemu server %s:%s connection: %s" % (self.host, self.port, conn))
            return conn
...
class VirtServer(object):
    ...
    def info(self):
        ...
        data = {
            ...
            "libver": libver,
            "hypervisor": {"type": conntype, "version": ver},
            "info": info,
            "uri": uri,
