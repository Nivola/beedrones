...
class KibanaEntity(object):
    ...
    def http_post(self, uri, data={}, files=None):
        ...
        try:
            ...
            if files is None:
                res = requests.post(uri, headers=headers, timeout=self.timeout, data=jsonDumps(data), verify=False)
            else:
                res = requests.post(uri, headers=headers, timeout=self.timeout, verify=False, files=files)
    ...
    def http_put(self, uri, data={}):
        ...
        try:
            ...
            res = requests.put(uri, headers=headers, timeout=self.timeout, data=jsonDumps(data), verify=False)
            if res.status_code in [400, 401, 403, 404, 405, 409]:
                output = res.json()
                self.logger.error('kibana http %s response: %s' % (method, truncate(output)))
                error = output['error'] + ' - ' + output['message']
