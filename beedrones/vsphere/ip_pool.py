...
class VsphereNetworkIpPool(VsphereObject):
    ...
    def _list(self):
        ...
        :return:
        """
        res = self.call('/api/2.0/services/ipam/pools/scope/globalroot-0', 'GET', '').get('ipamAddressPools')\
            .get('ipamAddressPool')
    ...
    def create(self, name, prefix=24, gateway='10.0.0.1', dnssuffix=None, dns1='8.8.8.8', dns2='8.8.4.4',
               startip='10.0.0.2', stopip='10.0.0.254'):
        """Create ip pool
    ...
    def update(self, oid, **kvargs):
        ...
        self.xml_set_key(data, kvargs, 'objectTypeName', default=orig_data['objectTypeName'], required=False)
        obj_type = et.SubElement(data, 'type')
        self.xml_set_key(obj_type, kvargs, 'typeName', default=dict_get(orig_data, 'type.typeName'), required=False)
        ...
        self.xml_set_key(data, kvargs, 'prefixLength', default=orig_data['prefixLength'], required=False)
        ...
        self.xml_set_key(ip_range_dto, kvargs, 'id', default=dict_get(orig_data, 'ipRanges.ipRangeDto.id'),
                         required=False)
        self.xml_set_key(ip_range_dto, kvargs, 'startAddress',
                         default=dict_get(orig_data, 'ipRanges.ipRangeDto.startAddress'), required=False)
        self.xml_set_key(ip_range_dto, kvargs, 'endAddress',
                         default=dict_get(orig_data, 'ipRanges.ipRangeDto.endAddress'), required=False)
        data = ensure_str(et.tostring(data))

        res = self.call('/api/2.0/services/ipam/pools/%s' % oid, 'PUT', data, headers={'Content-Type': 'text/xml'})
    ...
    def allocate(self, pool, static_ip=None):
        ...
        res = self.call('/api/2.0/services/ipam/pools/%s/ipaddresses' % pool, 'POST', data,
