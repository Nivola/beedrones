...
class VsphereDatastore(VsphereObject):
    ...
    def list(self):
        ...
        data = self.manager.collect_properties(view_ref=view, obj_type=vim.Datastore, path_set=props, include_mors=True)
        ...
        data2 = self.manager.collect_properties(
            view_ref=view, obj_type=vim.StoragePod, path_set=props, include_mors=True
        )
        data.extend(data2)
        return data
    ...
    def browse_files(self, datastore, path="/"):
        ...
        try:
            ...
            task = browser.SearchDatastore_Task(datastorePath="[%s] %s" % (datastore.name, path))
    ...
    def get_hosts(self, datastore):
        ...
        try:
            ...
            for host in hosts:
                host = host.key
                res.append({"name": host.name, "id": host._moId})
        except vmodl.MethodFault as error:
            self.logger.error(error.msg, exc_info=False)
            raise VsphereError(error.msg, code=0)
        return res
